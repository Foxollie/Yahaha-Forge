...
import net.foxolli3.yahaha.item.client.KorokWandThunderFullRenderer;
...
public class KorokWandThunderFull extends Item implements GeoItem {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);
    public KorokWandThunderFull(Properties pProperties) {
        super(pProperties);
    }
    ...
    public void initializeClient(Consumer<IClientItemExtensions> consumer) {
        consumer.accept(new IClientItemExtensions() {
            private KorokWandThunderFullRenderer renderer;

            @Override
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                if(this.renderer == null) {
                    renderer = new KorokWandThunderFullRenderer();
                }
    ...
    private void spawnParticles(ServerLevel level, BlockPos pos) {
        level.sendParticles(ModParticles.YELLOW_CHUCHU_BURST_PARTICLES_SMALL.get(), pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, 150, 1, 1, 1, 0);
        level.sendParticles(ParticleTypes.ELECTRIC_SPARK, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, 100, 1, 1, 1, 0);
    }
    ...
    private void spawnFredrick(BlockPos pos, ServerLevel level, Player player, InteractionHand hand) {
        ElectroFredrickMob fredrickMob = ModEntities.ELECTRO_FREDRICK.get().create(level);
        if (fredrickMob != null) {
    ...
    public void appendHoverText(ItemStack pStack, TooltipContext pContext, List<Component> pTooltipComponents, TooltipFlag pTooltipFlag) {
        if(Screen.hasShiftDown()) {
            pTooltipComponents.add(Component.translatable("tooltip.yahaha.korok_wand_thunder_full"));
        } else {
            pTooltipComponents.add(Component.translatable("tooltip.yahaha.chuchu_jelly"));
        }
        pTooltipComponents.add(Component.literal("§3FREDRICK §a[ELECTRO]§r"));
