...
public class RedChuchuJellyItem extends Item {
    ...
    public RedChuchuJellyItem(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResult useOn(UseOnContext pContext) {
        if (!pContext.getLevel().isClientSide()) {
            ...
            if (isKorokBlock == true) {
                if (item == Moditems.RED_CHUCHU_JELLY.get()) {
                    itemstack.shrink(1);
                    level.destroyBlock(positionClicked, false);
                    level.setBlock(positionClicked, ModBlocks.FIERY_KOROK_BLOCK.get().defaultBlockState(),1);
