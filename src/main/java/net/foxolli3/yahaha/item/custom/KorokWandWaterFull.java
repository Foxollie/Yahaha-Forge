...
public class KorokWandWaterFull extends Item implements GeoItem {
    ...
    private final Map<BlockPos, Integer> spawnAttemptTracker = new HashMap<>();
        ...
        private BlockPos findValidSpawnPosition(BlockPos startPos, ServerLevel level) {
            for (int dy = 0; dy <= 2; dy++) {
                ...
                boolean isAir = blockState.isAir() || blockState.is(Blocks.WATER);
                boolean hasSpaceAbove = blockAboveState.isAir() || blockAboveState.is(Blocks.WATER);
    ...
    public void appendHoverText(ItemStack pStack, TooltipContext pContext, List<Component> pTooltipComponents, TooltipFlag pTooltipFlag) {
        if(Screen.hasShiftDown()) {
            pTooltipComponents.add(Component.translatable("tooltip.yahaha.korok_wand_water_full"));
        } else {
            pTooltipComponents.add(Component.translatable("tooltip.yahaha.chuchu_jelly"));
        }
        pTooltipComponents.add(Component.literal("§3FREDRICK §b[HYDRO]§r"));
