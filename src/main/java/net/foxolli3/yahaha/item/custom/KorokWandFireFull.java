...
public class KorokWandFireFull extends Item implements GeoItem {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);
    public KorokWandFireFull(Properties pProperties) {
        super(pProperties);
    }
    ...
    public void initializeClient(Consumer<IClientItemExtensions> consumer) {
        consumer.accept(new IClientItemExtensions() {
            private KorokWandFireFullRenderer renderer;

            @Override
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                if(this.renderer == null) {
                    renderer = new KorokWandFireFullRenderer();
                }
    ...
    private void spawnParticles(ServerLevel level, BlockPos pos) {
        level.sendParticles(ParticleTypes.FLAME, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, 150, 1, 1, 1, 0);
        level.sendParticles(ParticleTypes.FALLING_LAVA, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, 150, 1, 1, 1, 0);
    ...
    private void spawnFredrick(BlockPos pos, ServerLevel level, Player player, InteractionHand hand) {
        PyroFredrickMob fredrickMob = ModEntities.PYRO_FREDRICK.get().create(level);
        if (fredrickMob != null) {
